...
app.post('/stock', function(req,res,next){
  ...
  if(req.body.stockTicker){
	 request( "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol="+req.body.stockTicker+"&interval=5min&apikey="+ credentials.stockApiKey, function(err, response, body){
		if(!err && response.statusCode < 400){
		  var response = JSON.parse(body);
		  var context = setContext(req,res);
		  context.stockMessage = JSON.stringify(response["Time Series (5min)"]);
